...
fn calc_consumption_int(report: &[Vec<bool>], rating_type: RatingType) -> u32 {
    ...
    for i in 0..matching[0].len() {
        ...
        match rating_type {
            RatingType::O2 => if set_reports.len() >= unset_reports.len() {
                matching = set_reports;
            } else {
                matching = unset_reports;
            }
            RatingType::CO2 => if set_reports.len() < unset_reports.len() {
                matching = set_reports;
            } else {
                matching = unset_reports;
            }
    ...
    assert_eq!(matching.len(), 1, "Should only have a single matching rating");
    let rating_raw = matching.first().unwrap();

    rating_raw.iter().rev().enumerate().map(|(i, is_set)| if *is_set {
        2u32.pow(i as u32)
    } else {
        0
    }).sum()
...
fn calc_consumption(report: Vec<Vec<bool>>) -> (u32, u32) {
    (calc_consumption_int(&report, RatingType::O2), calc_consumption_int(&report, RatingType::CO2))
}
...
mod test {
    ...
    #[test]
    fn test_parse_line1() {
        assert_eq!(parse_line("100110000110"), vec![true, false, false, true, true, false, false, false, false, true, true, false])
    }

    #[test]
    fn test_calc_consumption_int_o2() {
        assert_eq!(calc_consumption_int(&vec![
            parse_line("00100"),
            parse_line("11110"),
            parse_line("10110"),
            parse_line("10111"),

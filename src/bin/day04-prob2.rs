...
fn main() {
    let (order, boards) = std::fs::read_to_string("src/bin/day04.txt")
        .map(|file| {
    ...
    let winner = play_game(order, boards);
    println!("{:?}", winner);
}
...
fn play_game(order: Vec<u16>, boards: Vec<Board>) -> u32 {
    let mut remaining = boards;

    for val in order {
        let num_remaining = remaining.len();
        let mut completed = Vec::new();
        for (i, board) in remaining.iter_mut().enumerate() {
            board.mark(val);
            if board.is_bingo() {
                if num_remaining == 1 {

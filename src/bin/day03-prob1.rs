...
fn calc_consumption(report: Vec<Vec<bool>>) -> (u32, u32) {
    ...
    let gamma_bits = set_counts.into_iter().map(|count| count > threshold).collect::<Vec<bool>>();
    let gamma = gamma_bits.iter().rev().enumerate().map(|(i, is_set)| if *is_set {
        2u32.pow(i as u32)
    } else {
        0
    }).sum();
    ...
    let epsilon = epsilon_bits.rev().enumerate().map(|(i, is_set)| if is_set {
        2u32.pow(i as u32)
    } else {
        0
    }).sum();
...
mod test {
    ...
    fn test_calc_consumption() {
        assert_eq!(calc_consumption(vec![
            parse_line("00100"),
            parse_line("11110"),
            parse_line("10110"),
            parse_line("10111"),
